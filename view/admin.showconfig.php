<?php 
defined( '_VALID_MOS' ) or die( 'Direct Access to this location is not allowed.' ); 

echo $this->h->jQTab($viewParm['tagid']);
//    $cssFile = $mccom['mcasiteurl']."css/moscom.ui.tabs.css";
echo $this->h->cssLink($viewParm['cssfile']);   
$this->mccom['version']=$this->ut->moscomVersion($this->mccom['mcaabspath'].'comments.xml'); // more efficient here
 
?> 
<div id="<?=$viewParm['tagid'];?>">
  <ul>
  <li><a href="#moscomConfig"><span><?=_COM_A_CONFIG; ?></span></a></li>
  <li><a href="#moscomDonate"><span>Donate!</span></a></li>
  <li><a href="#moscomInstruction"><span>Instruction</span></a></li>
  <li><a href="#moscomSupport"><span>Support</span></a></li>
  <li><a href="#moscomMaillingList"><span>Mailling List</span></a></li>
  <li><a href="#moscomLicense"><span>License</span></a></li>
  <li><a href="#moscomVersion"><span>Version</span></a></li>
  <li><a href="#moscomCredit"><span>Credit</span></a></li>
  <li><a href="#moscomCSS"><span>CSS</span></a></li>      
  </ul>
  <div id="moscomConfig">
    <form action="index2.php" method="post" name="adminForm" class="adminForm" id="adminForm">
    <table width="90%" border="0" cellpadding="2" cellspacing="2" class="adminForm">
    <tr><td class="sectionname" colspan="3"><div align="center"><?php echo _COM_A_CONFIG; ?></div></td></tr>
    <tr align="center" valign="middle">
    <td width="17%">&nbsp;</td>
    <td width="17%"><strong><?php echo _COM_A_CURRENT_SETTINGS; ?></strong></td>
    <td width="66%"><strong><?php echo _COM_A_EXPLANATION; ?></strong></td>
    </tr>
    <?=$viewData?>
    <tr align="center" valign="middle"><td colspan="3" align="left" valign="top"></td></tr>
    </table>
    <input type="hidden" name="task" value="saveconfig" />
    <input type="hidden" name="option" value="<?=$this->mccom['me'];?>" />
    </form>      
  </div>     
  <div id="moscomDonate">
    <div class="sectionname">Donate!</div><p />
    <?=_COM_A_DONATE;?>
    <p />
    <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
    <input type="hidden" name="cmd" value="_s-xclick">
    <input type="image" src="https://www.paypal.com/en_US/i/btn/x-click-butcc-donate.gif" border="0" name="submit" alt="Make payments with PayPal - it's fast, free and secure!">
    <img alt="" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">
    <input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----">
    </form>
    <p>Help me keep this product free by donating a little money towards my costs. ~Thanks~</p>      
  </div>
  <div id="moscomInstruction">
    <div align="left">
      <div align="center"><font color="#FF9900"><h5><?=_COM_A_INSTRUCTIONS_DESC?></h5></font></div>
      <div>
        <font color="#DA3838"><b>Notes: Two reasons to install MosComBot:</b></font><p />
        1. Want to show comment link on front page.<br />
        2. Don&#39;t want to edit template, meaning to skip steps below! (edting template is now optional but still supported)
        Post Install Configuration<p />
        But if you want to edit the template, then read below!<br />
      </div>
      <div><strong><font color="#ffcc00"><?=_COM_A_IMPORTANT_NOTE?></font></strong></div>
      <div>
        <p><?=_COM_A_TEMPLATE?></p>
        <p /><?=_COM_A_CHANGE_TO?>
        <p />mosMainBody(); ?&gt; OR &lt;jdoc:include type=&quot;component&quot; /&gt;
        <p />
        <p>mosMainBody(); ?&gt;&lt;?php if (file_exists($mosConfig_absolute_path.&quot;/components/com_comments/comments.php&quot;)) { require_once($mosConfig_absolute_path.&quot;/components/com_comments/comments.php&quot;); } ?&gt;
        </p>
        OR
        <p>&lt;jdoc:include type=&quot;component&quot; /&gt;&lt;?php if (file_exists($mosConfig_absolute_path.&quot;/components/com_comments/comments.php&quot;)) { require_once($mosConfig_absolute_path.&quot;/components/com_comments/comments.php&quot;); } ?&gt;
        </p>
        <p><?=_COM_A_HAVE_FUN;?></p>
      </div>
    </div>    
  </div>
  <div id="moscomSupport" align="left">
    <?=$this->h->a("http://ongetc.com","Author Website");?> : Check out the forums or articles there first!<br /> 
    <?=$this->h->a("mailto:chanh.ong@gmail.com","Email Request");?> : I will do my best to reply! - May take Time<br />
    <?=$this->h->a("mailto:chanh.ong@gmail.com","Custom Development");?> : I will change code for a fee - Ask for details<br />
  </div>
    <div id="moscomMaillingList" align="center">
    <strong>Announcement list</strong><p /><?php echo _COM_A_KEEPUPTODATE; ?><p/>
    <form method="post"action="http://ongetc.com/mailman/subscribe/dev_ongetc.com" target="_blank">
      <input name="email" class="inputbox" value="<?=$viewParm['admin_email']?>" type="text" />
      <input name="submit" class="button" value="<?=_COM_A_SUBSCRIBE?>" type="submit" />
      <input name="action" value="subscribe" type="hidden">
      <input name="group_ids[]" value="8" type="hidden">
    </form>    
  </div>
  <div id="moscomLicense">
    <iframe frameborder="0" width="100%" height="600px" align="top" src="<?=$this->mccom['mcasiteurl'];?>license.php"></iframe>      
  </div>
  <div id="moscomVersion">
  <?php 
    $url=$this->mccom['mesite'].'static/moscom_version.php?myver='.$this->mccom['version'];
    $jsChkVersion="javascript:mc_openpopup('".$url."',485,180);"; 
    echo $this->h->yourVersion(_COM_A_VERSION,$this->mccom)
    ."<p />".$this->h->aLink($jsChkVersion,_COM_A_CHECK,'')
    ."<p />".$this->h->releaseVersion($viewParm, $this->mccom)
    //      ."<p />".$this->h->releaseVersion($this->mccfg, $this->mccom)      
    ; 
  ?>
  </div>
  <div id="moscomCredit">
    <?=$this->h->aLink("http://mambo-foundation.org","Mambo Team"); ?> & 
    <?=$this->h->aLink("http://joomla.org","Joomla Team"); ?> : For a wonderful CMS! - Thanks!<br />
    <p />... And Many contributors, Please see credit.txt for more detail    
  </div>
  <div id="moscomCSS">
    <p>You can now skin your comments by using provided moscom.css file in administrator/components/com_comments/css</p>
    And
    <p>You can now skin your admin tab by using provided moscom.ui.tabs.css file in administrator/components/com_comments/css</p>    
  </div>
</div>
<?=$this->h->footerAbout();?>