...
*/ 
/* dependencies: 
 + administrator/components/com_comments/json.php
 + administrator/components/com_comments/array_diff_key.php
 + includes/phpmailer/class.phpmailer.php
 + Mambo global variables
 + SHOULD NOT have any database releated call here!
*/
include_once("co.utils.class.php");
include_once("moscom.html.class.php");
class MoscomUtils extends CoUtils {
  var $h;
  var $adminside;
  var $_VERSION;
  var $my;
  ...
  var $mambo;
  var $timeOffset; 
  function MoscomUtils () { // contructor
    global $mainframe;
...
//    $this->mambo['ds']=$this->setDS();
  ...
  function sendAlert($mccom, $mccfg) {
    ...
    $mcmsg=_COM_A_NAME_SUB.": ".$mccom['name']."\n".
    		_COM_A_EMAIL_SUB.": ".$mccom['email']."\n".
  			_COM_C_HOMEPAGE. ": ".$mccom['homepage']."\n".
  			_COM_A_COMMENT. ": ".stripslashes($mccom['entry'])."\n".
  			$mccom['date']." - ".$mccom['time']."\n".
        "**Comments on Articles Script (MosCom) released by Chanh Ong base on Combo **";
    $mcmsgAdmin = _COM_A_IP. ": ".$mccom['ip']."\n". $mcmsg;   
    ...
    (($mccfg['review'] == "0") or ($mccom['gid']=="25")) 
      ? $reviewmsg = _COM_C_THANKS3 : $reviewmsg = _COM_C_THANKS4;
    return $reviewmsg; 
  }
 ...
 //   $this->ppDebug($mccom,"mccom");     
    $return = '0';
    $moscomts = $this->getParam('MosComTS','post');
    if (!empty($moscomts)) {
      $seconds = 60*10;	// 10 minutes timeout
    	$str = $mccom['meip'].$mccom['articleid'].$moscomts;
    	if(isset($_COOKIE[$mccom['me']]) && $_COOKIE[$mccom['me']] == md5($str)) 
        $return = $moscomts; // 1241217007 return the token
      else if (((int)$moscomts + $seconds) < mktime()) 
        $return = "-1";    
      else 
        $return = '0';
    }
  ...
  function mailAdmin($to, $atitle, $mcmsg, $from) {
  	$to = $to."\n";
  	$subject = _COM_C_NEW_COM." ".$this->mambo['siteurl'];
  	$msg = _COM_C_HAVE_NEW." $atitle \n\n".$mcmsg.
        "\n\n"._COM_C_LOGIN."\n"._COM_C_QUICKLINK.": ".$this->mambo['siteurl']."administrator";
  	$this->mosMail( $from, $from , $to, $subject, $msg );//Send to admin
  }
  function mailUser($to,$atitle,$mcmsg,$from) {
    $to = $to."\n";
  	$subject = _COM_C_THANKS." ".$this->mambo['siteurl']."...";
  	$msg = _COM_C_THANKS2.": $atitle \n"._COM_C_ADMIN_REV.".  "._COM_C_ENTERED."\n\n".$mcmsg.
        "\n\n"._COM_C_VISIT.": ".$this->mambo['siteurl'];
...
//       $this->usingOnce($this->mambo['siteabspath'].'includes/phpmailer/class.phpmailer.php');
  ...
  function moscomVersion($filename) {
  ...
  */
  ...
  function getOffsetDate($iVar) {
			...
			return _COM_C_DATE_ON.' '.mosFormatDate($offset_date[0], _DATE_FORMAT_LC).' '. _COM_C_DATE_AT.' '.$offset_date[1]; 
  }
