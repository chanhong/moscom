...
function mc_openpopup(iurl,iwidth,iheight){
	...
	var urlPop = iurl;
	moscompopup=window.open(urlPop,"popFrameless",+s);
	moscompopup.blur();
	window.focus();
	moscompopup.resizeTo(windowW,windowH);
	moscompopup.moveTo(windowX,windowY);
	moscompopup.focus();
}

// begin of bbclass class
function bbclass(objName) {
this.moscomhelptext = null;
...
//		img_help    = "Insert image: [img]http://image_url[/img]";
		...
		moscomhelptext=eval("document."+myhelpbox);
		moscomhelptext.value = eval(help + "_help");
