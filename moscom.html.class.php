...
class MoscomHTML extends CoHTML {
*/
include_once("moscom.jquery.class.php");
class MoscomHTML extends MoscomJQuery {
...
<div class='bbCodeToHide' id='bbcodetohide".$id."'>".$this->bbCodeHelp()."</div>";
  ...
  function releaseVersion($mccfg, $mccom) {
    $moscom_version="moscom_version.php";	// productiont
    $url=$mccom['mesite'].'static/'.$moscom_version; // production
    $url.='?myver='.$mccom['version'];
    ($mccfg['updates'] == "1")  
    ? $moscom_release_version = '<iframe src="'.$url.'" height="33px" frameborder="1"></iframe>'
    : $moscom_release_version = "";
    return $moscom_release_version;
  }
...
"
    ...
    return $js.'<input id="bbutton" class="moscomBBBUTTON" value="B" style="font-weight: bold;" onclick="javascript:'.$bb.'.b('.$tt.',\'b\')" title="[b]Bolded text[/b]" type="button" />
    <input id="ubutton" class="moscomBBBUTTON" value="U" style="text-decoration: underline;" onclick="javascript:'.$bb.'.b('.$tt.',\'u\')" title="[u]Underlined text[/u]" type="button" />
    <input id="ibutton" class="moscomBBBUTTON" value="I" style="font-style: italic;" onclick="javascript:'.$bb.'.b('.$tt.',\'i\')" title="[i]Italicized text[/i]" type="button" />
    <input id="colorbutton" class="moscomBBBUTTON" value="Color" onclick="javascript:'.$bb.'.b('.$tt.',\'color\')" title="Font color: [color=red]text[/color] Tip: you can also use color=#FF0000" type="button" />
    <input id="centerbutton" class="moscomBBBUTTON" value="Center" onclick="javascript:'.$bb.'.b('.$tt.',\'center\')" title="[center]center text[/center]" type="button" />
    <input id="quotebutton" class="moscomBBBUTTON" value="Quote" onclick="javascript:'.$bb.'.b('.$tt.',\'quote\')" title="To quote: [quote]quoted text[/quote]" type="button" />
    <input id="linkbutton" class="moscomBBBUTTON" value="URL" onclick="javascript:'.$bb.'.b('.$tt.',\'url\')" title="Insert URL: [url]http://url[/url] or [url=http://url]URL text[/url]" type="button" />
    <input id="codebutton" class="moscomBBBUTTON" value="Code" onclick="javascript:'.$bb.'.b('.$tt.',\'code\')" title="Code display: [code]code[/code]" type="button" />
    <input id="sizebutton" class="moscomBBBUTTON" value="Size" onclick="javascript:'.$bb.'.b('.$tt.',\'size\')" title="Font size: [size=x-small]small text[/size]" type="button" />
   <input id="'.$helpbox.'" name="'.$helpbox.'" size="73" class="moscomHelpBox" value="Tip: Mouse over or click on above button for bbcode." readonly="readonly" type="text" title="Tip: Mouse over or click on above button for bbcode." /><br  />
';
  }  

  function noComments($mccfg) { 
  ($mccfg['use_divcss']=="0")
  ? $output = '<p /><div align="center"><b>'._COM_C_NO_COM.'</b></div>'
  : $output = '<p class="moscomFORMMsg">'._COM_C_NO_COM.'</p>';  
  return $output;
  }
  function footerAbout() { return $this->copyright(_COM_A_MOS_BY." Chanh Ong"); }
  function copyright($iText="") { 
    return $iText."<br /><div style='font-size:smaller' align='center'><a href='http://ongetc.com'>&copy;2008 MosCom</a></div>";
  }  
  function cfgListFromSchemaArray($mcarray,$mccfg) {  
    $aYesNo = array('0'=>_COM_A_NO, '1'=>_COM_A_YES);  
