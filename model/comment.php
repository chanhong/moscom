...
*/ 
include_once("moscom.php");
class MoscomCommentModel extends MoscomModel {
  ...
  function dbSelectValue($aid) { return $this->dbQuerySelectValue("Select entry from #__content_comments where id=".$aid); }
  function dbSelectArticleComments($mccfg, $id) {
  	if ($mccfg['new_1st'] =="1" ) { $lorderby="order by id DESC"; } else {$lorderby="order by id ASC"; }
  	return $this->dbQuerySelect("SELECT * FROM #__content_comments where articleid=".$id." AND published=1 ".$lorderby);
  } 
...
//    $oneComment['articleid'] = $mccom['articleid'];  
  ...
  function dbSelect($search, $limitstart, $limit) {
    ...
    return $this->dbQuerySelect("SELECT * FROM #__content_comments" . $where ." ORDER BY id DESC LIMIT ".$limitstart.", ".$limit);
  }
  function getCount($search) { 
    $where = $this->dbSetFilter($search);    
    $result = $this->dbQueryExec("SELECT id FROM #__content_comments ".$where." ORDER BY id DESC");
  ...
  FROM `#__content` as b, `#__content_comments` as a LEFT JOIN `#__content` as c ON (c.id=a.articleid)
  ...
  function setActionSql($task, $cid) {
    ...
    if (count( $cid )) {
      ...
      switch ($task) {
      case "remove":
        $qry = "DELETE FROM #__content_comments WHERE id IN ($cids)";
        break;
      case "publish":
        $qry = "UPDATE #__content_comments SET published='1' WHERE id IN ($cids)";
        break;
      case "unpublish":
        $qry = "UPDATE #__content_comments SET published='0'  WHERE id IN ($cids)";
        break;
      }
    }
    return $qry;
...
//    $this->ut->ppDebug($oneComment,__METHOD__.'Check input');         
    if ($this->ut->isInputValid($oneComment,$mccfg)) {   
      ...
      if (MOSCOM_DEBUG) { $oneComment['entry'] = "(".$mccom['gaid'].")=>".$oneComment['entry']; }      
...
//      $output = $this->ut->sendAlert($oneComment, $mccfg);    
  ...
  function dbEditSave($option) {
  	...
  	$this->dbQueryExec('UPDATE #__content_comments SET entry="'.$comment.'" WHERE id = "'.$cid.'"');
  	$mosmsg = "Comment id ".$cid." is saved";
  	mosRedirect( 'index2.php?option='. $option, $mosmsg);
  }  
//INSERT INTO `#__content_comments` (`articleid`, `id`, `entry`, `name`, `email`, `homepage`, `date`, `time`, `published`) VALUES (1, 0, 'This is a test entry against article 1', 'Chanh Ong', 'chanh.ong@gmail.com', 'http://ongetc.com', 'Today', 'Now!', '0') 
...
//     $this->ppDebug($iArray,__METHOD__.'=>fldList=>'.$fldList);     
    (!empty($fldList) and !empty($iArray['articleid'])) ? $sql = "INSERT INTO #__content_comments SET id=0, ".$fldList : $sql = ""; 
...
//  	$redirectlink="index2.php?option=com_comments&task=config"; // stay at config screen
  	$redirectlink="index2.php?option=com_comments"; // force a reload to get new value from db
  ...
  function dbInsertTable() {
    $dbInit = false;
      ...
      $this->dbQueryExec("INSERT INTO `#__content_comments` ( `id`) value (0)");   
  ...
  function dbCreateTable() {
    return "CREATE TABLE IF NOT EXISTS `#__content_comments` (
...
";
  ...
  function dbAlterTable($type, $diffArray) {
    ...
    !empty($fldsList) ? $sql = "ALTER TABLE `#__content_comments` ".$fldsList.";" : $sql = "";
  ...
  function dbUpdateTable($iArray) {
    ...
    $id = $this->getRid();
    !empty($iArray) ? $fldList = $this->fldsListFromArray($iArray) : $fldList = "";
    (!empty($fldList) and !empty($id)) ? $sql = "UPDATE #__content_comments SET ".$fldList." where id='".$id."'" : $sql = "";  
    if (!empty($sql)) $this->dbQueryExec($sql);    
  }  

  function dbSelectTable() {
  	$rows = $this->dbQuerySelect("SELECT * FROM `#__content_comments` limit 1");
